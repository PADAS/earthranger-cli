...
def _canonical_schema(schema) -> object:
    ...
    everywhere it appears — schema-level icon_id/image_url that ER injects on
    GET; empty-string description/placeholder/default and empty
    conditionalDependents/conditions on any property or UI field (top-level,
    allOf branches, or collection items); and the builder's random condition
    ids. All are wire-equivalent to omitting the key."""
    schema = normalize_v2_schema(copy.deepcopy(schema))
    if not isinstance(schema, dict):
        return schema
    schema.pop("icon_id", None)
    schema.pop("image_url", None)
    _strip_echo_noise(schema)
    return schema


_EMPTY_STRING_NOISE = ("description", "placeholder", "default")


def _strip_echo_noise(node) -> None:
    if isinstance(node, dict):
        for key in _EMPTY_STRING_NOISE:
            if node.get(key) == "":
                del node[key]
        if node.get("conditionalDependents") == []:
            del node["conditionalDependents"]
        ...
        if conditions == []:
            ...
            for cond in conditions:
                if isinstance(cond, dict):
                    cond.pop("id", None)  # internal, randomly generated by ER's builder
        for value in node.values():
            _strip_echo_noise(value)
    elif isinstance(node, list):
        for value in node:
            _strip_echo_noise(value)
